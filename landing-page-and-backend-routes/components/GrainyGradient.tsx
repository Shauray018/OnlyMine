  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      for (let i = 0; i < Math.min(activeRipples.length, 10); i++) { // Max 10 ripples
        ...
        // Convert to normalized coordinates (-1 to 1) using actual viewport dimensions
...
const fragmentShader = `
  ...
  // Multi-color gradient function - Dark theme with green accents
  vec3 multiColorGradient(float t) {
    // Define your color palette - blacks with subtle green highlights
    vec3 colors[7];
    colors[0] = hexToRgb(20.0, 100.0, 35.0);   // dark green (reduced from bright green)
    colors[1] = hexToRgb(15.0, 75.0, 28.0);    // darker green
    colors[2] = hexToRgb(12.0, 60.0, 22.0);    // dark green
    colors[3] = hexToRgb(8.0, 40.0, 15.0);     // very dark green
    colors[4] = hexToRgb(4.0, 18.0, 8.0);      // almost black green
    colors[5] = hexToRgb(2.0, 8.0, 4.0);       // near black
    colors[6] = hexToRgb(0.0, 0.0, 0.0);       // pure black
